...
{
    s_T.begin();
    if (index >= 0 && index < 7) T_measure = s_T.getTempCByIndex(T_index);
...
//====================================================================
//Methods
float DataSnsr::getT_Measure(DallasTemperature &s_T)
{
    return T_measure;
}
...
void DataSnsr::feedT_data(DallasTemperature &s_T)
{
    if (T_index >= 0 && T_index < 7)
    {
        T_measure = s_T.getTempCByIndex(T_index);
        for (auto &i:T_data)
        {
            if (i == 999.99) 
            {
